...
<%
	...
	// HTML 출력용 (태그 깨짐 방지)
	String safeSearch = rawSearch != null ? rawSearch.replace("&", "&amp;")
	                                      .replace("<", "&lt;")
	                                      .replace(">", "&gt;")
	                                      .replace("\"", "&quot;")
	                                      .replace("'", "&#x27;")
...
<div class="container mt-4">
	...
	<form method="get" action="bbs.jsp" class="form-inline mb-3">
		<input type="text" name="search" class="form-control mr-2"
			placeholder="Search..." value="<%= safeSearch %>">
	...
	<table class="table table-hover">
		...
		<tbody>
			...
			ArrayList<Bbs> list = (rawSearch != null && !rawSearch.trim().equals("")) 
				? bbsDAO.searchList(rawSearch, pageNumber)
			...
			<tr>
				...
				<td><%=b.getUserID()%></td>
				<td><%=b.getBbsDate().substring(0, 11) + b.getBbsDate().substring(11, 13) + "시 " + b.getBbsDate().substring(14, 16) + "분"%></td>
			</tr>
			<%
			}
			}
	...
	<div class="d-flex justify-content-between">
		<div>
			...
			if (pageNumber != 1) {
			%>
			<a href="bbs.jsp?pageNumber=<%=pageNumber - 1%>&search=<%=safeSearch%>" class="btn">Previous</a>
			<%
			}
			if (bbsDAO.nextPage(pageNumber + 1)) {
			%>
			<a href="bbs.jsp?pageNumber=<%=pageNumber + 1%>&search=<%=safeSearch%>" class="btn">Next</a>
			<%
			}
			%>
		</div>
