...
<body>
    <%
        ...
        String secretPassword = multipartRequest.getParameter("secretPassword");
        if (isSecret == null) {
        ...
        if ("Y".equals(isSecret)) {
            if (secretPassword == null || secretPassword.trim().equals("")) {
                PrintWriter script = response.getWriter();
                script.println("<script>");
                script.println("alert('비밀글 비밀번호를 입력하세요.');");
                script.println("history.back();");
                script.println("</script>");
                return;
            }
